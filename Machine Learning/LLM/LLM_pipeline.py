...
# ── FastAPI app ───────────────────────────────────────────────
app = FastAPI(title="Honey In The Cloud — LLM Pipeline")
...
async def process_rollup(payload: RollupPayload):
    ...
    result = await run_pipeline(rollup=rollup, current_WAF_rules=current_WAF_rules)
...
# ── Pipeline runner ───────────────────────────────────────────
async def run_pipeline(rollup: dict, current_WAF_rules: list) -> dict:
    messages = [
          {
            "role": "system",
            "content": (
...
def create_consolidated_firewall_rules(cluster_groups: list[dict]) -> dict:
    ...
    print(f"\n── Rule budget: {len(manual_rules)} manual + {len(auto_rules)} [AUTO] = {len(existing_rules)}/{RULE_BUDGET} used")
        ...
        if existing:
            ...
            try:
                r = requests.patch(
                    ...
                    json={"expression": updated_expr, "description": description, "enabled": True}
                )
                r.raise_for_status()
...
def build_prompt(current_WAF_rules: list) -> str:
    ...
    **Skip** clusters with severity LOW, INFO, or where jira_priority is null.
    ...
    ## TOOL CALL ORDER
    Execute tools in this exact order to avoid incomplete Jira references:
    1. query_knowledgebase — for each HIGH/CRITICAL cluster (can batch into 1-2 queries)
    2. create_jira_ticket  — one per qualifying cluster
    3. create_consolidated_firewall_rules — exactly once, covering all qualifying clusters
